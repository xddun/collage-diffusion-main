...
def all_collage(experiment_name, collage_prompt, layer_list, ca, textual_inversion, inversion_step_list, device_id,
                input_seed,
    ...
    if textual_inversion:
        ...
        for layer in layer_list:
            if layer.ftc is not None:
                match_token = max(collage_prompt.find(layer.image_str + " "),
                                  collage_prompt.find(layer.image_str + ","))
                if match_token >= 0:
                    collage_prompt = collage_prompt[:match_token] + new_token_options[i] + " " + collage_prompt[
                                                                                                 match_token:]
            i += 1

    print("加了标记的collage_prompt：", collage_prompt)
...
room_images = all_collage("toyBox2_step1", collage_prompt, layer_list, ca=True, textual_inversion=True,
                          inversion_step_list=[1000] * 4, device_id=0,
                          input_seed=None, w_pos=[0.0, 0.5, 0.2, 0.2], w_neg=[0.5, 0.5, 0.5, 0.5], img2img_strength=0.8,
                          show_all=True,
